...
developer_chain = (
    RunnableLambda(lambda x: print(">>> 개발자 체인 실행 중...") or x)
    | make_chain("당신은 파이썬 개발자입니다.")
)
chef_chain = (
    RunnableLambda(lambda x: print(">>> 요리사 체인 실행 중...") or x)
    | make_chain("당신은 요리사입니다.")
)
general_chain = (
    RunnableLambda(lambda x: print(">>> 일반 사용자 체인 실행 중...") or x)
...
questions = [
    ...
    "파이썬 리스트 정렬 코드 알려줘.",
    "오늘 날씨 어때?",
    "된장찌개 파이썬으로 끓이는 요리 알려줘." # 브랜치 내에서 가장 먼저 만나는 조건이 실행된다. 이 경우 개발자 체인이 실행될 것이다.
]
