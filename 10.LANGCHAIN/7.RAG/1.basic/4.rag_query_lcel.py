...
retriever = store.as_retriever(search_kwargs={"k":3}) # 검색 시 유사한 문서 3개 반환하도록 설정

prompt = ChatPromptTemplate.from_template(
    """
    아래 문서만 참고하여 질문에 답하시오.\n\n
    문서:\n{context}\n\n
    질문:{question}
    """
)
...
chain = (
    {
        ...
        "question": RunnablePassthrough() # 질문은 그대로 다음 파이프라인으로 그대로 전달
    }
    | prompt
    | llm
    | parser
...
print(f"챗봇 응답: {chain.invoke(question)}")
