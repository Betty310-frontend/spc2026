...
wiki_ko = WikipediaQueryRun(
    api_wrapper=WikipediaAPIWrapper(
            lang='ko', top_k_results=3,
            doc_content_chars_max=200
        ),
        name="wiki_ko",
        description="한국어 위키피디어. 한국에서 일어난 사건을 중심으로 정보를 제공합니다."

)

wiki_en = WikipediaQueryRun(
    api_wrapper=WikipediaAPIWrapper(
            lang='en', top_k_results=3,
            doc_content_chars_max=200
        ),
        name="wiki_en",
...
system_prompt = """
    ...
    - 검색 결과가 한번에 안 나올 경우, 유사어(유의어) 등으로 변경해서 재시도 할 수 있음
...
questions = ['파이썬은 누가 만든 언어야?', 'What is artificial intelligence?', '세종대왕은 누구인가?']
    ...
    for m in result['messages']:
        if hasattr(m, 'tool_calls') and m.tool_calls:
            for call in m.tool_calls:
                print(f"-> 사용한 도구: {call['name']} ({call['args']})")
        if m.type == 'tool':
            print(f"-> 결과: {m.content[:100]}...")

print(f"\n[최종 답변]: {result['messages'][-1].content}") # 배열의 마지막 메시지가 최종 답변이 될 가능성이 높음