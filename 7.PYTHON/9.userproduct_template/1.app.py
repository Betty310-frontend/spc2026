...
def get_users(user_id=None):
    if user_id is None:
        return render_template('user.html', users=users.values())
    user = users.get(user_id)
    if user:
        return render_template('user.html', users=[user], user_id=user_id)
    return render_template('user.html', users=[], user_id=user_id)
...
def get_products():
    ...
    if product_id is not None:
        ...
        if product:
            return render_template('product.html', products=[product], product_id=product_id)
        return render_template('product.html', products=[], product_id=product_id)
    if product_name is not None:
        filtered_products = [p for p in products.values() if p['name'].startswith(product_name)]
        return render_template('product.html', products=filtered_products, product_name=product_name)
    return render_template('product.html', products=products.values())
